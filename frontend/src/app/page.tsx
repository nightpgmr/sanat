  ...
  return (
    <>
      ...
      <section className="hero-gradient relative overflow-hidden">
        ...
        <div className="absolute -left-32 top-20 h-64 w-64 rounded-full bg-gold/10 blur-3xl" />
        <div className="absolute -right-32 bottom-10 h-80 w-80 rounded-full bg-emerald/5 blur-3xl" />

        <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 sm:py-28 lg:px-8">
          <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
            {/* Text */}
            <div className="animate-fade-up">
              <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-gold/25 bg-gold/8 px-4 py-2 text-sm font-medium text-gold">
            ...
            <div className="relative hidden h-[420px] lg:block">
              <div className="animate-float absolute right-0 top-0 w-64 rounded-3xl border border-gold/20 bg-surface-elevated p-6 shadow-2xl glow-gold">
                <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-gold/15">
                  <Icon name="formulation" className="h-6 w-6 text-gold" />
                </div>
                ...
                <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-border">
                  <div className="h-full w-4/5 rounded-full bg-gradient-to-l from-gold to-gold-light" />
                </div>
                <p className="mt-2 text-xs text-gold">۸۰٪ تکمیل پروژه نمونه</p>
              </div>

              <div className="animate-float-delay absolute bottom-8 right-32 w-56 rounded-3xl border border-emerald-500/20 bg-surface-elevated p-5 shadow-xl">
                <div className="mb-2 flex items-center gap-2">
                  <span className="text-2xl">🌸</span>
                  <div>
                    <p className="text-sm font-bold">زعفران سرگل</p>
                    <p className="text-xs text-emerald-400">آماده صادرات</p>
                  </div>
                </div>
                <div className="flex gap-1">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <Icon key={i} name="star" className="h-3 w-3 text-gold" filled />
                  ))}
                </div>
              </div>

              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-52 rounded-3xl border border-border bg-surface p-5 shadow-xl">
                <p className="text-xs font-medium text-muted">پروژه‌های موفق</p>
                <p className="mt-1 text-3xl font-black text-gold">+۵۰۰</p>
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-6 px-4 sm:gap-10 sm:px-6">
          {trustBadges.map((badge) => (
            <div key={badge.label} className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-gold/20 bg-gold/8">
                <Icon name="quality" className="h-4 w-4 text-gold" />
              </div>
              <div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {quickActions.map((action) => (
              <Link
                key={action.href}
                href={action.href}
                className={`card-hover group relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br ${action.color} p-6`}
              >
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-surface-elevated/80 transition-colors group-hover:bg-gold/15">
                  <Icon name={action.icon as "consult"} className="h-6 w-6 text-gold" />
                </div>
                <h3 className="text-lg font-bold text-foreground group-hover:text-gold">
                  {action.title}
                </h3>
            ...
            {processSteps.map((step, i) => (
              <div key={step.step} className="relative">
                {i < processSteps.length - 1 && (
                  <div className="absolute -left-3 top-8 hidden h-px w-6 bg-gradient-to-l from-gold/40 to-transparent lg:block" />
                )}
                <div className="rounded-3xl border border-border bg-surface-elevated p-6">
                  <span className="text-4xl font-black text-gold/30">{step.step}</span>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {services.map((service) => (
              <Link
                ...
                className="card-hover group rounded-3xl border border-border bg-surface p-6"
              >
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-gold/10 transition-colors group-hover:bg-gold/20">
                  <ServiceIcon slug={service.slug} className="h-6 w-6 text-gold" />
                </div>
                <h3 className="text-base font-bold text-foreground group-hover:text-gold">
                  {service.shortTitle}
                </h3>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {productCategories.map((cat, i) => (
              <Link
                ...
                className={`card-hover group relative overflow-hidden rounded-3xl border border-border bg-surface-elevated p-7 ${
                  i === 0 ? "sm:row-span-1 lg:col-span-1" : ""
                }`}
              >
                <div className="absolute -left-4 -top-4 h-24 w-24 rounded-full bg-gold/5 blur-2xl transition-all group-hover:bg-gold/10" />
                ...
                <h3 className="relative mt-4 text-xl font-bold text-foreground group-hover:text-gold">
                  {cat.title}
                </h3>
                <p className="relative mt-2 text-sm text-muted">{cat.description}</p>
                <div className="relative mt-5 flex items-center gap-1 text-sm font-medium text-gold">
                  مشاهده
                  <Icon name="chevron" className="h-4 w-4" />
                </div>
              </Link>
      ...
      <section className="relative overflow-hidden px-4 py-20 sm:px-6 lg:px-8">
        <div className="absolute inset-0 bg-gradient-to-bl from-gold/8 via-transparent to-emerald/5" />
        <div className="relative mx-auto max-w-7xl">
          <div className="overflow-hidden rounded-[2rem] border border-gold/20 bg-surface">
            <div className="grid lg:grid-cols-2">
              ...
              <div className="grid grid-cols-2 gap-px bg-border">
                {[
                  ...
                  <div
                    key={c.country}
                    className="flex flex-col items-center justify-center bg-surface-elevated p-8 transition-colors hover:bg-surface"
                  >
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {testimonials.map((t) => (
              <blockquote
                key={t.name}
                className="card-hover rounded-3xl border border-border bg-surface p-7"
              >
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {caseStudies.map((study) => (
              <article
                key={study.slug}
                className="card-hover overflow-hidden rounded-3xl border border-border bg-surface-elevated"
              >
                <div className="h-2 bg-gradient-to-l from-gold via-gold-light to-emerald" />
                <div className="p-6">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {blogPosts.map((post) => (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}/`}
                className="card-hover group flex flex-col rounded-3xl border border-border bg-surface p-6"
              >
                <span className="w-fit rounded-full bg-gold/10 px-2.5 py-0.5 text-xs font-semibold text-gold">
                  {post.category}
                </span>
                <h3 className="mt-3 flex-1 text-base font-bold text-foreground group-hover:text-gold line-clamp-2">
                  {post.title}
                </h3>
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="space-y-3">
            {faqs.slice(0, 5).map((faq, i) => (
              <details
                key={i}
                className="group overflow-hidden rounded-2xl border border-border bg-surface-elevated transition-colors open:border-gold/30"
              >
                <summary className="flex cursor-pointer items-center justify-between gap-4 px-6 py-5 font-semibold text-foreground">
                  {faq.question}
                  <Icon name="chevron" className="faq-chevron h-5 w-5 shrink-0 text-gold" />
                </summary>
      ...
      {/* ── Final CTA ── */}
      <section className="px-4 py-20 sm:px-6 lg:px-8">
        <div className="relative mx-auto max-w-4xl overflow-hidden rounded-[2rem] border border-gold/25 bg-gradient-to-br from-gold/15 via-surface to-surface p-10 text-center sm:p-14">
          ...
          <div className="relative">
            ...
            <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
              ...
              <a
                ...
                className="inline-flex items-center gap-2 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-8 py-4 text-base font-semibold text-emerald-400 transition-all hover:bg-emerald-500/20"
              >
